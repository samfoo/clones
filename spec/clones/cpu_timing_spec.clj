...
(describe "The timing of instructions on the NES's 6502 2A03/7 CPU"
  (describe "execute-with-timing"
    ...
    (describe "branching instructions"
      ...
      (for [[op-code [branch-cpu no-branch-cpu]] expected-times]
        (let [op (op-by-opcode op-code)
              timing (timing-by-opcode op-code)
              n (:name (meta op))]
          (describe (format "$%02x (%s)" op-code n)
            (it (str "shouldn't count as crossing a page unless the "
              ...
              (let [on-page-boundary (assoc branch-cpu :pc 0xff)
                    will-branch (second (io-> on-page-boundary
                                              (io-write 0xff 0x10)))
                    [cycles _] (execute-with-timing will-branch op timing)]
                (should= 3 cycles)))

            (it "should take 4 cycles when branching to a new page"
              (let [will-cross (second (io-> branch-cpu
                                             (io-write 0x90 0)))
                    [cycles _] (execute-with-timing will-cross op timing)]
                (should= 4 cycles)))

            (it "should take 2 cycles when not branching"
              (let [[cycles _] (execute-with-timing no-branch-cpu op timing)]
                (should= 2 cycles)))

            (it "should take 3 cycles when branching"
              (let [[cycles _] (execute-with-timing branch-cpu op timing)]
                (should= 3 cycles)))))))

    (describe "without page-crossing or branching"
      (def expected-times {0x69 2 0x65 3 0x75 4 0x6d 4 0x7d 4 0x79 4 0x61 6 0x71 5
      ...
      (for [[op-code expected] expected-times]
        (let [op (op-by-opcode op-code)
          ...
          (it (str "should take " expected " cycles when executing op code "
            ...
            (let [[cycles _] (execute-with-timing cpu op timing)]
              (should= expected cycles))))))))
